...
function init_globals ()
{
    ...
    bspeed=100          # badguy speed
    btimer=$(timer)     # badguy speed timer
    bc=0        # badguy counter
    cb=0        # current badguy
    ...
    cht=$(timer) # animation timer
    anim_speed=200  # animation speed

    SPEED_CONST=1000000  # Constant to multiply timer by.
...
function main ()
{
    ...
    do
        ...
        # Move badguy
        if [[ $(($(timer) - $btimer)) -ge $(($bspeed * $SPEED_CONST)) && $bc -gt 0 ]]
        then
            # Chase player
            bmx=0
            bmy=0
        ...
        # Animate characters
        if [[ $(($(timer) - $cht)) -ge $(($anim_speed * $SPEED_CONST)) && $bc -gt 0 ]]
        then
